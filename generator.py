...
# Новые темы дней: вовлекающие и релевантные для Москвы/области
DAYS_THEMES = {
...
Мы — компания «ГлобалТракГарант». Работаем с 2015 года, специализируемся на рефрижераторных перевозках по Москве и Московской области. Наш тон — уверенный, но дружеский, без официоза. Мы гордимся отсутствием срывов и строгим контролем температуры. В каждом посте подчёркиваем надёжность и экспертность.
...
def collect_facts(topic):
    ...
    if topic == "новости_компании":
        ...
        if news:
            ...
            # Если новостей нет, заменяем тему на полезный пост
            facts = {
        ...
        # Без привязки к конкретным трассам, общая картина по Москве и области
        facts = {
